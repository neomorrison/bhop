/* ============================== [INPUT] ==============================
   ...
   tick at a time (see consumeLook).  That matters: air acceleration is a
   per-tick calculation, so applying a whole frame's turn before the first
...
export function initInput(canvas, hooks = {}) {
  ...
  // Scroll-jump. Every bhop player binds the wheel to +jump, because a wheel
  // notch is a far more precise way to hit the one tick you are on the ground
  // than a key press is. It is timing, never direction.
  ...
  canvas.addEventListener('click', () => { if (!suspended && !mouse.locked) canvas.requestPointerLock(); });
 ...
 * Auto-hop only ever answers "when", never "where".
 */
