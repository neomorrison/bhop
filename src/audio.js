/* ============================== [AUDIO] ==============================
   ...
   Every cue is tied to a movement event, so you can hear a clean hop.     */
...
/** Wind bed follows speed — the loudest feedback channel you have at 800 u/s. */
export function updateAudio(speed, airborne) {
  if (!ctx || !windGain) return;
  const on = SETTINGS.sound ? 1 : 0;
  const k = Math.max(0, Math.min(1, (speed - 240) / 700));
  const target = on * k * k * 0.22 * (airborne ? 1 : 0.55);
  windGain.gain.setTargetAtTime(target, ctx.currentTime, 0.08);
  windFilter.frequency.setTargetAtTime(320 + k * 1500, ctx.currentTime, 0.1);
}
